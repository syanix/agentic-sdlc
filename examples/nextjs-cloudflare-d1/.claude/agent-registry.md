...
|-------|-------|-------|------|-------------|
| be-dev | sonnet | Cloudflare Workers + Hono + D1 | Backend development | Implements API endpoints, Workers routes, and D1 data operations |
| fe-dev | sonnet | Next.js (App Router) | Frontend development | Implements frontend components, pages, and state management |
| be-tester | sonnet | Vitest + miniflare | Backend testing | Writes unit and integration tests for Workers and D1 logic |
| fe-tester | sonnet | Playwright | Frontend E2E testing | Writes end-to-end tests for frontend user journeys |

### Project Agent Details

**be-dev**
- Implements Hono route handlers, middleware, and service logic
- Manages D1 schema definitions with Drizzle ORM and SQL migrations
- Configures Workers bindings (D1, KV, R2, Durable Objects) in `wrangler.toml`
- Ensures proper error handling, input validation, and edge runtime compatibility

**fe-dev**
- Implements UI components and page layouts with Next.js App Router
- Manages Server Components, Client Components, and Server Actions
- Follows Next.js conventions for routing, data fetching, and caching
- Ensures responsive design and accessibility with Shadcn/ui and Tailwind CSS

**be-tester**
- Writes unit tests for services and utilities using Vitest
- Writes integration tests for Workers routes using miniflare
- Mocks D1 bindings and external service calls
- Ensures adequate test coverage for all backend logic
...
- Ensures adequate test coverage for frontend user flows
